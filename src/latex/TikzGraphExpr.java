...
public class TikzGraphExpr {
	...
	private static final double freshNodeDist = 1;
	//! Horizontal spacing of links in a S link
	private static final double distanceStatement = 0.1;
	
	public static String draw(GraphString phrase, List<String> words, TypeReduction red, ExprResolver res)
	{
		int n = phrase.size();
		...
		for(int i = 0; i < n-1; i++)
		{
			...
			{
				output += drawGraphExpr(phrase.getPattern(i), res, pos, pos[i], graphExprDist);
			}
		}
	...
	private static String drawGraphExpr(PatternExpr p, ExprResolver res, double[] pos, double nodePos, double height)
	{
		if(p.isNode())
			return drawNodeExpr((NodeExpr)p,res,pos,nodePos,height);
		else if(p.isStmt())
			return drawStmtExpr((StmtExpr)p,res,pos,nodePos,height);
		
		return "";
	}
	
	private static String drawNodeExpr(NodeExpr n, ExprResolver res, double[] pos, double nodePos, double height)
	{
		if(n.isVar())
		{
		...
		{
			return "\\node[circle,draw] at ("+nodePos+","+(height+freshNodeDist)+") (fresh) {\\_};\n"+
		...
		else if(n.isMorph())
		{
			...
			String start = "\\node[rectangle,draw] at ("+nodePos+","+(height+freshNodeDist)+")"+
					" (morph) {$"+(m.isReify ? "\\beta" : "\\alpha")+"$};\n"+
					"\\draw ("+nodePos+","+height+") -- (morph);\n";
			return start+drawStmtExpr(m.triple,res,pos,nodePos,height+freshNodeDist+0.28);
		}
		else if(n.isLiteral())
		{
			return "\\draw ("+nodePos+","+height+") -- ("+nodePos+","+(height+0.5)+");\n";
		}
		return "";
	}
	
	private static String drawStmtExpr(StmtExpr s, ExprResolver res, double[] pos, double nodePos, double height)
	{
		if(s.isTriple())
		{
			...
			return drawNodeExpr(t.subj, res, pos, pos_1, height) +
					drawNodeExpr(t.prop, res, pos, pos_2, height) +
					drawNodeExpr(t.obj, res, pos, pos_3, height);
		}
		else if(s.isVar())
		{
